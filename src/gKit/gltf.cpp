...
std::vector<GLTFInstances> GLTFScene::instances( ) const
{
    std::vector<GLTFInstances> instances = std::vector<GLTFInstances>();
    std::vector<int> indexs = std::vector<int>(meshes.size(), -1);
    for(unsigned i= 0; i < nodes.size(); i++){
        if(indexs[nodes[i].mesh_index] == -1){
            indexs[nodes[i].mesh_index] = instances.size();
            GLTFInstances newInstance;
            newInstance.mesh_index = nodes[i].mesh_index;
            instances.push_back(newInstance);
        }
        instances[indexs[nodes[i].mesh_index]].transforms.push_back(nodes[i].model);
    }
    return instances;
}
